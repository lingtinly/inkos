...
const BASE_INSTRUCTIONS = [
  'You are the text-generation backend for InkOS, a fiction-writing application.',
  'Do only writing, planning, editing, summarizing, and other text-only creative work.',
  'Do not inspect files, run commands, browse, edit files, or perform coding-agent work.',
  'Follow the supplied writing instructions and conversation context.',
...
function detectChineseLengthTarget(messages) {
  const latestUser = [...messages].reverse().find((m) => m.role === 'user')?.content || '';
  const patterns = [
    /(?:约|大约|控制在|写|生成|不少于|不低于|至少|最多|不超过)?\s*(\d{2,5})\s*(?:个)?(?:中文汉字|汉字|中文字|中文字符|字)(?!母)/,
    /(\d{2,5})\s*字(?:左右|以内|上下)?/
  ];
  for (const pattern of patterns) {
    const match = latestUser.match(pattern);
    if (match) {
      const target = Number(match[1]);
      if (Number.isFinite(target) && target >= 20 && target <= 50000) return target;
    }
  }
  return null;
}
...
function splitContext(messages) {
  ...
  const targetHanChars = detectChineseLengthTarget(messages);
  const lengthContract = targetHanChars
    ? [
        `CHINESE LENGTH CONTRACT: The latest user asks for ${targetHanChars} 字.`,
        `Interpret this as approximately ${targetHanChars} Chinese Han characters (汉字), not English words, tokens, bytes, or total Unicode characters.`,
        `Aim for ${Math.round(targetHanChars * 0.95)}-${Math.round(targetHanChars * 1.05)} Han characters; do not intentionally stop far below the target.`,
        'Punctuation does not count toward the Han-character target.'
      ].join(' ')
    : '';
  return {
    system,
...
class AppServer {
  ...
  async _start() {
    ...
    child.stderr.on('data', (d) => { this.stderr = (this.stderr + d.toString()).slice(-16000); });
    child.on('exit', (code, signal) => {
      if (this.child !== child) return;
      this.child = null;
      this.ready = null;
      this.fail(new BridgeError(`app-server exited ${code ?? '?'}${signal ? '/' + signal : ''}: ${compact(this.stderr)}`, 'app_server_exited', 503));
    });
    child.on('error', (e) => this.fail(new BridgeError(`app-server start failed: ${e.message}`, 'app_server_start_failed', 503)));

    const initialized = await this.request('initialize', {
      clientInfo: { name: 'inkos_codex_bridge', title: 'InkOS Codex Bridge', version: '0.5.0' },
  ...
  onLine(line) {
    ...
    if (!threadId && params.turn?.id) threadId = this.byTurn.get(params.turn.id);
  ...
  async generate(model, context, onDelta, signal) {
    ...
    try {
      const started = await this.request('thread/start', {
        ...
        developerInstructions: context.system || 'Produce only the requested text.'
      }, 30000);
      ...
      const output = new Promise((resolve, reject) => {
        ...
        this.turns.set(threadId, { resolve, reject, timer, text: '', final: '', error: '', delta: onDelta });
      });

      const abort = () => {
        if (threadId && turnId) this.request('turn/interrupt', { threadId, turnId }, 5000).catch(() => {});
      };
  ...
  fail(error) {
    ...
    this.pending.clear();
    for (const state of this.turns.values()) { clearTimeout(state.timer); state.reject(error); }
    this.turns.clear();
    this.byTurn.clear();
  }
...
async function execFallback(model, context, onDelta, signal) {
  ...
  try {
    ...
    const args = [
      ...
      '--color', 'never', '--sandbox', 'read-only', '--output-last-message', outputFile,
      '--model', model, '-c', `model_reasoning_effort="${effort(model)}"`, '-'
    ];
    const result = await spawnExec(args, prompt, cwd, signal);
    let text = '';
    try { text = (await readFile(outputFile, 'utf8')).trim(); } catch { text = result.stdout.trim(); }
...
async function chat(req, res, body) {
  ...
  const record = (transport, finalText, error) => {
    const ended = now();
    ...
    stats.last = {
      ...
      outputHanChars: countHan(text),
      ttftMs: firstTokenAt ? firstTokenAt - started : null,
      totalMs: ended - started,
      error: error ? compact(error.message || error) : null
    };
    console.log(`[inkos-codex-perf] model=${model} transport=${transport} targetHan=${targetHanChars ?? '-'} outputHan=${stats.last.outputHanChars} ttft=${stats.last.ttftMs ?? '-'}ms total=${stats.last.totalMs}ms`);
  };
  ...
  if (body.stream) {
    ...
    try {
      ...
      record(transport, result.text, null);
    } catch (error) {
      record(transport, streamedText, error);
  ...
  try {
    const result = await generate(model, messages, null, ctl.signal);
    firstTokenAt = now();
    json(res, 200, {
      id,
    ...
    record(result.transport, result.text, null);
  } catch (error) {
...
async function handler(req, res) {
  ...
  if (req.method === 'GET' && ['/health', '/v1/health', '/diagnostics', '/v1/diagnostics'].includes(path)) {
    return json(res, 200, {
      ok: true,
  ...
  if (req.method === 'POST' && ['/chat/completions', '/v1/chat/completions'].includes(path)) {
    return chat(req, res, await readBody(req));
  }

  return json(res, 404, { error: { message: `Unsupported route: ${req.method} ${path}`, type: 'invalid_request_error', code: 'route_not_found' } });
}
...
if (process.argv.includes('--check')) {
  const started = now();
  generate(DEF_MODEL, [{ role: 'user', content: 'Reply with exactly CODEX_BRIDGE_OK' }], null)
    .then((result) => {
  ...
  const server = http.createServer((req, res) => handler(req, res).catch((error) => {
    ...
    json(res, error.status || 500, { error: { message: error.message || String(error), type: 'codex_cli_error', code: error.code || 'bridge_error' } });
  }));

  server.listen(PORT, HOST, () => {
    console.log(`[inkos-codex-bridge] v5 listening on http://${HOST}:${PORT}/v1`);
    console.log(`[inkos-codex-bridge] default=${DEF_MODEL}; Terra/Luna=none; Sol=low; Chinese length contract enabled`);
